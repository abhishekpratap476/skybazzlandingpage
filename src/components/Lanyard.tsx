  ...
  useEffect(() => {
    ...
    if (card.current && j1.current && j2.current && j3.current && fixed.current) {
      try {
        ...
        j1.current.setTranslation({ x: 0.5, y: 4, z: 0 }, true);
        j1.current.setLinvel({ x: 0, y: 0, z: 0 }, true);

        j2.current.setTranslation({ x: 1, y: 4, z: 0 }, true);
        j2.current.setLinvel({ x: 0, y: 0, z: 0 }, true);

        j3.current.setTranslation({ x: 1.5, y: 4, z: 0 }, true);
        j3.current.setLinvel({ x: 0, y: 0, z: 0 }, true);

        card.current.setTranslation({ x: 2.2, y: 4, z: 0.2 }, true);
        card.current.setLinvel({ x: -1.5, y: -2, z: 0.3 }, true);
        card.current.setAngvel({ x: 0.2, y: 3.5, z: -0.2 }, true);
      } catch {
        // Safe fallback
      }
    }

...
const Masonry = ({
  ...
  const getInitialPosition = (item: any, index?: number) => {
  ...
  const containerHeight = useMemo(() => {
    if (!grid || grid.length === 0) return 400;
    return Math.max(...grid.map(item => item.y + item.h)) || 400;
  }, [grid]);

  const hasMounted = useRef(false);

  useIsomorphicLayoutEffect(() => {
    if (!imagesReady || !triggerAnimation) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [imagesReady, triggerAnimation, stagger, animateFrom, blurToFocus, duration, ease, grid.length]);
